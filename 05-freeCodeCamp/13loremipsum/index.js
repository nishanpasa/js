...
// if hit enter after putting number, trigger the generate btn
...
// generating lorem paras as per the value input 
generateBtn.addEventListener('click', function () {
    if (paraInput.value === '' || paraInput.value > 9 || paraInput.value < 1 ) {
        randomNumber = Math.floor(Math.random()*paraArray.length);
        loremContainer.innerHTML = paraArray[randomNumber];
    }
    else{        
    let paragraphs = paraArray.slice(0, paraInput.value);
    paragraphs = paragraphs.map((para) => `<p>${para}</p>`);
    paragraphs = paragraphs.join("");
    console.log(paragraphs);

    loremContainer.innerHTML = paragraphs;

    // for (let i = 0; i < paraInput.value; i++) {
    //     loremContainer.innerHTML += `${paraArray[i]}`;        
    // }
    }
})
